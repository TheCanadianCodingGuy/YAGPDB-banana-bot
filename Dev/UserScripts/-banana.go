...
{{else}}
    ...
    {{if $isSlip}}
        ...
        {{else}}
            ...
            {{end}}
        {{end}}
        {{if $isCrashActive}}{{$addedSlips = 0}}{{$body = printf "🚨 **MARKET CRASH ACTIVE!** %s's slip was worthless! +0 Slips. (%d market crash rolls remaining)" $userName (toInt $global.crash)}}
