{{$now := currentTime}}{{$stM := (newDate $now.Year (toInt $now.Month) 1 0 0 0)}}{{$ss := $stM.AddDate 0 1 0}}{{$ssu := $ss.Unix}}{{$lsu := sub $ssu 21600}}
...
🚫 **The season has ended!**
    ...
    **Next season starts at:** <t:{{$ssu}}:F> (<t:{{$ssu}}:R>)
    {{return}}{{end}}
{{$ks := "banana_slips"}}{{$keyData := "banana_data"}}{{$keyGlobal := "banana_global"}}{{$prestigeGlobal := "prestige_global"}}{{$cdr := 18000}}{{$milc := 20}}{{$malc := 70}}{{$oMarketCrash := 7}}{{$oHalving := 1}}{{$oRankSwap := 4}}{{$oTurbo := 12}}{{$oPlosion := 14}}{{$oOily := 14}}{{$oMythic := 6}}{{$oCosmic := 11}}{{$rawName := .User.Username}}
...
{{if gt (toInt $myPrestige) 0}}{{$userName = printf "(🏆%d) %s" (toInt $myPrestige) $userName}}{{end}}
...
{{if eq .User.ID $.User.ID}}{{$currentRank = $rankTracker}}{{$myIndex = $i}}{{end}}{{end}}
...
{{$slipType := "ns"}}{{if $isSlip}}{{$luckyChance := $malc}}{{if le $currentRank 20}}{{$range := sub $malc $milc}}
...
{{$roll := randInt 1 101}}
{{if $wasTurbo}}{{$roll = randInt (add $tOily 1) 101}}{{end}}
{{if le $roll $tCrash}}
{{$slipType = "mcs"}}
...
{{if ge $targetIdx (len $te)}}{{$targetIdx = sub (len $te) 1}}{{end}}{{else}}
{{$targetIdx = sub $myIndex $offset}}
{{if lt $targetIdx 0}}{{$targetIdx = 0}}{{end}}{{end}}
...
{{$body = printf "🚀 **RANK SWAP!** %s vaulted upwards and swapped their **%d** slips with <@%d>'s **%d** slips! 🏎️💨" $userName $oldSlips $targetID $targetSlips}}{{else}}
{{$body = printf "📉 **RANK SWAP!** %s fumbled and swapped their **%d** slips with <@%d>'s **%d** slips from below! 🤼‍♂️" $userName $oldSlips $targetID $targetSlips}}{{end}}{{else}}
{{$slipType = "gs"}}
{{$isSwap = false}}{{$multiplier = 2}}
{{$body = printf "😮 %s tried to swap, but no one was there! A **Golden Peel** was found instead." $userName}}{{end}}
