...
{{/* Fetch Global Season Number */}}
{{$season := 1}} {{/* Default fallback if not found */}}
{{with (dbGet 0 "banana_global")}}
    {{$globalData := dict .Value | sdict}}
    {{with ($globalData.Get "season")}}{{$season = .}}{{end}}
{{end}}
...
{{$embed := cembed 
    "title" (printf "📊 Season %v Stats: %s" $season $userName)
    "description" $desc
    "color" 16766720 
