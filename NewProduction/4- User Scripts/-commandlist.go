{{ $embed := sdict
    ...
    "fields" (cslice
        ...
        (sdict "name" "📊 Player Data" "value" "• `-seasonstats [@user]`: View detailed analytics including total slips, longest flip streaks, and a breakdown of every special peel type encountered for the **current season**.\n• `-lifetimestats [@user]`: View the life-time detailed analytics including total slips, longest flip streaks, and a breakdown of every special peel type encountered.\n• `-what`: A comprehensive deep-dive into the game's mechanics, event odds, and the prestige system." "inline" false)
